...
void PRF_(unsigned char *password, unsigned char *salt_key, int pwd_len){
    LoadMacKey(salt_key);
    GenerateMAC(pwd_len, password, MAC);
}

int main()
{
    unsigned char password[] = {0x01,0x02,0x03,0x04,0x05,0x06,0x01,0x02,0x03,0x04,0x05,0x06,0x01,0x02,0x03,0x04,0x05,0x06};
    unsigned char salt_key[] = {0x01,0x02,0x03,0x04,0x05,0x06,0x01,0x02,0x03,0x04,0x05,0x06,0x01,0x02,0x03,0x04};
    int len = 18;
    PRF_(password,salt_key,len);
    int i = 0;
    for(i = 0;i<16;i++){
        printf("%.2x ",MAC[i]);
    }
